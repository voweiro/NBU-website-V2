...
export default function Navbar() {
    ...
    academics: [
      {
        ...
        subItems: [
          {
            href: "/academics/faculties/management-social-sciences",
            label: "Management and Social Sciences",
          },
          {
            href: "/academics/faculties/it-computing",
            label: "IT & Computing",
          },
          { href: "/academics/faculties/engineering", label: "Engineering" },
          {
            href: "/academics/faculties/health-sciences",
  ...
  const isActiveDropdown = (key: string): boolean => {
    return (
      ...
      navigationItems[key].some(
        (item) =>
          isActiveLink(item.href) ||
          (item.subItems &&
            item.subItems.some((sub) => isActiveLink(sub.href))),
      )
    );
  };
  ...
  return (
    <>
      <nav
        ref={navRef}
        className="fixed top-0 lg:top-4 left-0 lg:left-1/2 lg:-translate-x-1/2 w-full lg:w-[80%] z-50 rounded-none lg:rounded-[2rem] border-b lg:border border-slate-100 py-3 bg-white/95 backdrop-blur-md shadow-xl"
      >
        <div className="max-w-screen-2xl mx-auto px-6 lg:px-12">
          <div className="flex justify-between items-center relative">
            ...
            <Link
              href="/"
              className="flex items-center gap-3 group z-10 shrink-0"
            >
              ...
              <div className="flex items-center gap-1 p-1 rounded-2xl bg-slate-900/5 drop-shadow-sm">
                ...
                {Object.entries(navigationItems).map(([key, items]) => (
                  ...
                  >
                    ...
                    >
                      {key}
                      <ChevronDownIcon className="h-3 w-3 ml-1.5 transition-transform duration-300 group-hover/nav:rotate-180" />
                      {isActiveDropdown(key) && (
                        <motion.span
                          layoutId="activeNav"
                          className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-[#e62627]"
                        ...
                        >
                          {items.map((item, idx) => (
                            <div
                              ...
                              className="relative"
                              onMouseEnter={() =>
                                item.subItems && handleSubMouseEnter(item.label)
                              }
                            ...
                            >
                              ...
                              >
                                <div className="flex items-center">
                                  <span
                                    className={`w-1 h-1 rounded-full mr-3 transition-all ${isActiveLink(item.href) ? "bg-[#e62627] scale-150" : "bg-slate-300 group-hover:bg-slate-400"}`}
                                  />
                                  {item.label}
                                </div>
                                {item.subItems && (
                                    ...
                                    >
                                      ...
                                      {item.subItems.map(
                                        (
                                          ...
                                          >
                                            <span
                                              className={`w-1 h-1 rounded-full mr-3 transition-all ${isActiveLink(sub.href) ? "bg-[#e62627] scale-150" : "bg-slate-300 group-hover:bg-slate-400"}`}
                                            />
                                            {sub.label}
                                          </Link>
                                        ),
                    ...
                    return (
                      <div key={key} className="space-y-1">
                        ...
                        >
                          <span>{key}</span>
                          <div
                            className="flex items-center justify-center w-5 h-5 rounded-full border border-white/20 transition-transform duration-300"
                            style={{
                              transform: isOpen
                                ? "rotate(180deg)"
                                : "rotate(0deg)",
